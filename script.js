...
async function callotify(message) {
...
// Update your callNotify function to use service worker
function callNotify(message) {
  ...
  // Method 1: Use service worker (preferred)
  if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
    ...
    // Also show via SW registration
    if (Notification.permission === 'granted') {
      navigator.serviceWorker.ready.then(registration => {
        registration.showNotification('CipherX', {
          body: message,
          icon: 'ciph.png',
          badge: 'ciph.png',
          vibrate: [100, 50, 100]
...
function registerServiceWorker() {
  if ('serviceWorker' in navigator) {
    // IMPORTANT: Use current page's directory as scope
    const swPath = window.location.pathname.includes('/CipherX/') 
      ? './sw.js' 
      : './sw.js';
...
function setupInstallPrompt() {
  ...
  window.addEventListener('appinstalled', () => {
    ...
    toast.success('CipherX is now installed!', 3000);
  });
}
